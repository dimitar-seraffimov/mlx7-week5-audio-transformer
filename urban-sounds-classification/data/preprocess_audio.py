...
class AudioPreprocessor:
  ...
  def __init__(self, sample_rate=44100, n_mels=64, n_fft=1024, hop_length=256, n_patches=3):
  ...
  def __call__(self, wave_form):
    ...
    spectrogram = spectrogram.unsqueeze(0) # (n_mels, time)
    # normalize
    spectrogram = (spectrogram - self.mean()) / (spectrogram.std() + 1e-6)
    # patch into 3 temporal segments
    patches = self._split_into_patches(spectrogram)

    return patches
  
  # split spectrogram into 3 temporal segments -> output is a tensor of shape (n_patches, n_mels, patch_length)
  def _split_into_patches(self, spectrogram):
    time_steps = spectrogram.shape[1]
    patch_size = time_steps // self.n_patches
    patches = []

    for i in range(self.n_patches):
      start = i * patch_size
      end = (i + 1) * patch_size if i < self.n_patches - 1 else time_steps
    ...
    patches = torch.stack(patches) # (n_patches, n_mels, patch_length)
    return patches